...
開始サンプル値・終了サンプル値・ループ時開始サンプル値は `AudioClip.frequency` を基準に解釈されます。Audio Import Settings によってサンプリングレートが変換される場合、設定したサンプル値と意図した再生位置がずれます。すべての AudioClip ・すべてのビルドターゲットでサンプリングレートが一致していることを確認してください。
