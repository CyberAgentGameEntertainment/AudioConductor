...
- [セットアップ](#セットアップ)
  ...
  - [インストール](#インストール)
- [注意事項](#注意事項)
  - [サンプル値パラメータと AudioClip のインポート設定](#サンプル値パラメータと-audioclip-のインポート設定)
- [アセットを作成する](#アセットを作成する)
...
`End sample` に 0 以下の値を指定すると、「クリップの終端まで再生する」という意味のセンチネル値として扱われます。  
...
- ピッチ
- ピッチ反転 (Pitch invert)
- 基準サンプルレート (Reference sample rate)
- キューリスト
...
### サンプル値パラメータと AudioClip のインポート設定

開始サンプル値・終了サンプル値・ループ時開始サンプル値は、`AudioClip.frequency` を基準とするサンプルインデックスとして保存されています。Audio Import Settings で **Preserve Sample Rate** を使用しない場合や、プラットフォームがランタイムにオーディオをリサンプリングする場合は、オーディオが異なる周波数でデコードされるため、同じサンプルインデックスでも再生される時間位置が変わり、意図したタイミングからずれます。

`referenceSampleRate` はキューシートのサンプル値を記述した際の基準サンプルレートを記録します。AudioConductor はランタイムでクリップの実際のデコード周波数に合わせて各サンプルインデックスをスケーリングします。データ量を抑えるためトラック単位ではなくキューシート単位で管理しています。キューシート内のクリップ間で周波数がばらつくと 1 つの値で基準サンプルレートを記録できないため、プロジェクト内のすべてのサウンドファイルのサンプルレートを統一してください。

`referenceSampleRate` が未設定のキューシートが存在する状態でプロジェクトを開くと、マイグレーションダイアログが表示され、現在のクリップ周波数を自動適用できます。また、キューシートのパラメータペインの **Reference Sample Rate** セクションの **Apply** ボタンからいつでも適用することもできます。
...
**Reference Sample Rate** フィールドは読み取り専用で表示されます。未設定（0）の場合は警告が表示されます。キューシート内のすべての AudioClip が同じサンプルレートを持つ場合、**Apply** ボタンが有効になり、クリップの周波数を値として設定できます。  
...
- トラック: `Start sample` ≥ `End sample`（`End sample` が 0 より大きいときのみチェック。`End sample` が 0 以下は「クリップの終端まで再生する」ことを意味するセンチネル値のため、このチェックはスキップされます）。
- トラック: `Loop start sample` ≥ `End sample`（ループ有効なトラックのみ、`End sample` が 0 より大きいときのみチェック。`End sample` が 0 以下の場合はこのチェックはスキップされます）。
...
- トラック: `End sample` が AudioClip のサンプル数を超えている。
...
**警告**:
- キューシート: キューリストが空。
- キューシート: `referenceSampleRate` が未設定で、かつ AudioClip が割り当てられているトラックが存在する。Audio Import Settings でリサンプリングが行われる場合や、プラットフォームのデコード周波数がクリップのネイティブレートと異なる場合にサンプル位置がずれる可能性があります。
